...
class StrPicture:
    ...
    def operation(self):
        ...
        for i in range(HEIGHT):
            ...
            txt += '\r\n'
        # 字符画输出到文件
        with open('strimage/' + self.IMG[:self.IMG.rfind('.')]+'.txt','w') as file:
            file.write(txt)
if __name__ == '__main__':
    if len(sys.argv) < 2:
        ...
        # 根据输入的文件夹遍历取出图片
        picture_list = os.listdir(picturedir)
        for picturefile in picture_list:
            strpicture = StrPicture(picturedir + picturefile)
            # 利用协程来处理图片
            g = gevent.spawn(strpicture.operation)
            g.join()
